    ...
    public function getRegExtSavingGraduatesByOccupation($occupation_id)
    {
        $result = SavingGraduate::select(
            DB::raw('
                        ...
                        sum(regular_level1_saving + regular_level2_saving +
                            regular_level3_saving + regular_level4_saving +
                            regular_level5_saving) as reg_total_saving,
                        sum(extension_level1_saving + extension_level2_saving +
                            extension_level3_saving + extension_level4_saving +
                            extension_level5_saving) as ext_total_saving'))
            ->where('report_date_id', $this->report_date_id)
            ->where('occupation_id', $occupation_id)
            ->whereIn('institution_id', Institution::select('id')
                ->where('region_id', $this->region_id)
